    ...
    foreach ($samAccountName in $accounts) {
        try {
            ...
            if ($externAccount) {
                ...
                if ($LogTextbox) {
                    Write-Log -Message "extensionAttribute11 wurde für $samAccountName auf x gesetzt (Extern Account)." -Color "green" -LogTextbox $LogTextbox
                }
            }
            ...
            if ($LogTextbox) {
                Write-Log -Message "Suche Mailbox-Datenbank für $($samAccountName) (Site=$site)" -Color "blue" -LogTextbox $LogTextbox
            }
            ...
            if (-not $mailboxDatabases) {
                if ($LogTextbox) {
                    Write-Log -Message "KEINE passende Mailbox-Datenbank gefunden für $($samAccountName). Muster=$mailboxSite" -Color "red" -LogTextbox $LogTextbox
                }
                continue
            }
            ...
            if (-not $userMailBoxDB) {
                if ($LogTextbox) {
                    Write-Log -Message "Keine geeignete Mailbox-Datenbank gefunden für $($samAccountName)." -Color "red" -LogTextbox $LogTextbox
                }
                continue
            }
            ...
            if ($LogTextbox) {
                Write-Log -Message "Primäre DB: $($userMailBoxDB.Name), Archiv-DB: $archiveDbName" -Color "green" -LogTextbox $LogTextbox
            }
